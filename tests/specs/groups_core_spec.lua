...
return {
...

  {
    ...
    run = function()
      ...
      local desired, conflicts, states = groups.project(head, {})
      t.deepEq(conflicts, {})
...

  {
    ...
    run = function()
      ...
      local desired = groups.project(head, { deletes = { [2] = true } })
      t.truthy(desired[1])
      t.falsy(desired[2])
    end,
  },

  {
    ...
    run = function()
      ...
      local _, _, states = groups.project(head, { adds = { [99] = note(1920, { pitch = 72 }) } })
...

  {
    ...
    run = function()
      ...
      local desired, conflicts, states = groups.project(head, inst)
...

  {
    ...
    run = function()
      ...
      local desired, conflicts = groups.project(head, inst)
...

  {
    ...
    run = function()
      ...
      local desired = groups.project(head, inst)
...

  {
    ...
    run = function()
      ...
      local add  = { evType = 'note', chanDelta = 0, key = 0, ppq = 0,
                     pitch = 64, vel = 100 }
      local desired, conflicts, states = groups.project(head, { adds = { [99] = add } })
...

  {
    ...
    run = function()
      ...
      local desired, _, states = groups.project(head, inst)
      t.truthy(desired[1])
...

  {
    ...
    run = function()
      ...
      local d1 = groups.project(head, {})
      local d2 = groups.project(head, {})
...

  {
    ...
    run = function()
      local ops = groups.reconcile({ [1] = note(0) }, {})
      t.eq(#ops, 1)
      t.eq(ops[1].op, 'add')
      t.eq(ops[1].vuid, 1)
    end,
...

  {
    ...
    run = function()
      local ops = groups.reconcile({}, { [1] = { uuid = 555, groupEvt = note(0) } })
      t.eq(#ops, 1)
      t.eq(ops[1].op, 'del')
      t.eq(ops[1].uuid, 555)
    end,
...

  {
    ...
    run = function()
      ...
      local ops = groups.reconcile(desired, current)
      t.eq(#ops, 1)
      t.eq(ops[1].op, 'set')
      t.eq(ops[1].uuid, 555)
...

  {
    ...
    run = function()
      local current = { [1] = { uuid = 555, groupEvt = note(0) } }
      t.eq(#groups.reconcile({ [1] = note(0) }, current), 0)
    end,
  },
...

  {
    ...
    run = function()
      t.deepEq(groups.deriveAssign(note(0, { pitch = 62 }), 'pitch', 71), { base = 62, value = 71 })
    end,
  },

  {
    ...
    run = function()
      t.deepEq(groups.deriveAssign(nil, 'pitch', 71), { base = nil, value = 71 })
    end,
  },
...

  {
    ...
    run = function()
      t.eq(groups.streamId({ evType = 'note', key = 0 }), 'note:0')
      t.eq(groups.streamId({ evType = 'note', key = 2 }), 'note:2')
      t.eq(groups.streamId({ evType = 'cc',   key = 74 }), 'cc:74')
      t.eq(groups.streamId({ evType = 'pb' }), 'pb:0')                -- no key -> 0
      t.truthy(groups.streamId({ evType = 'cc', key = 1 })
            ~= groups.streamId({ evType = 'cc', key = 74 }))
    end,
  },
...

  {
    ...
    run = function()
      ...
      for _, originPpq in ipairs{ 0, 480, 1920 } do
        ...
        t.falsy(groups.inRect(rect, -1,  0, n))                      -- before span
        t.truthy(groups.inRect(rect, 0,   0, n))                     -- span start (inclusive)
        t.truthy(groups.inRect(rect, 479, 0, n))                     -- last ppq in span
        t.falsy(groups.inRect(rect, 480, 0, n))                      -- span end (exclusive)
      end
    end,
  },

  {
    ...
    run = function()
      ...
      t.truthy(groups.inRect(rect, 0, 0, note(0)))
      t.falsy(groups.inRect(rect, 0, 1, note(0)))                    -- chanOffset 1 absent
    end,
  },

  {
    ...
    run = function()
      ...
      t.truthy(groups.inRect(rect, 0, 0, { evType = 'cc', key = 74, ppq = 0 }))
      t.falsy(groups.inRect(rect, 0, 0, note(0)))                    -- note:0 not selected
    end,
  },

  {
    ...
    run = function()
      ...
      t.truthy(groups.inRect(rect, 0, 0, note(0)))
      t.truthy(groups.inRect(rect, 0, 0, { evType = 'cc', key = 74 }))
      t.falsy(groups.inRect(rect, 0, 1, note(0)))                    -- ch1 carries no notes
      t.truthy(groups.inRect(rect, 0, 1, { evType = 'cc', key = 1 }))
    end,
  },
}
