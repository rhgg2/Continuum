...
`am:projectTracks`, `am:tracksTakes`, `am:trackSlots`,
`am:slotForTake`, `am:keyForSlot`, `am:newMidiSlot`,
`am:newAudioSlot`, `am:deleteSlot`, `am:renameSlot`, plus the folded
`am:takesUsing`/`am:reswingAll`. The placement and per-take edit API
listed in `design/arrange.md` (`dropInstance`, `duplicateTake`,
`moveTake`, `resizeTake`, `trimStart`, `trimEnd`, `deleteTake`) is
deferred to phases 4-5 alongside the base36 command scope and mouse
drag handlers; building them now would ship code with no caller.
