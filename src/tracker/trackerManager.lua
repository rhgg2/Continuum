...
--shape: frame.channels[chan].parked = { { evType='note', chan, uuid, ppq, endppq, endppqC, pitch, vel, detune, sample, delay, lane, [fx] }, ... } -- render-ready off-take replace cells (endppq is the authored ceiling the view edits, endppqC the render clip realiseParked derives)
--shape: frame.channels[chan].parkedCC = { { evType='cc', chan, cc, ppq, val, shape, [tension] }, ... } -- off-take cc-replace render cells
--shape: frame.channels[chan].parkedPb = { { evType='pb', chan, ppq, val (=cents), cents, shape, [tension] }, ... } -- off-take pb-replace render cells
--shape: frame.channels[chan].parkedPA = { { evType='pa', chan, pitch, ppq, vel, [rpb] }, ... } -- off-take PA cells; rebuildPA re-projects them into the host note column
...
-- `channels` map swaps each pass, and the operations that seat cells in it travel alongside.
...
do
  ...
  -- Field write on a seated cell: renew only where the value actually moves, or the tail walk's
  -- restamp renews every bounded lane every pass. Cells are self-describing, so (chan, lane) is here.
  function frame.setCell(cell, field, value)
    if cell[field] == value then return end
    frame.renewLane(cell.chan, cell.lane)
    cell[field] = value
  end
...
local function channelStreams(chan, startL, endL, pbBase, ccBases)
  local cols = frame.channels[chan].columns
  local pas, ats = {}, {}
...
-- One host cell's span end: authored (or take-end) ceiling, clipped to its strict-next lane onset.
local function clippedSpanEnd(cell, takeLenL)
  local ceil = (cell.endppq == nil or cell.endppq == util.OPEN) and takeLenL or math.min(cell.endppq, takeLenL)
  local succ = nextLaneOnset(frame.channels[cell.chan].columns.notes[cell.lane].events, cell.ppq)
...
-- A parked spec's span end: authored endppq unclipped by lanes; OPEN falls to take length.
...
--invariant: ordered note, pb, then cc ascending
...
local function windowSet(fxRegions, noteFxSpans, parkedSpecs)
  ...
  -- computeNoteFxSpans already found every on-take fx host, parked hosts declared out -- no re-filter.
  -- Sort (chan, lane, ppq): the whole-column scan's emission order, so perTarget stays G4-stable.
  local noteHosts = {}
  for host, spanEnd in pairs(noteFxSpans) do util.add(noteHosts, { host = host, endppq = spanEnd }) end
...
--invariant: a seated record marks via setCell, off-take direct: no lane renews for a cell it lacks
...
local function reconcilePCsForChan(chan, records, sink, seedSpans)
  local existing = {}
  for _, e in ipairs((frame.channels[chan].columns.pc and frame.channels[chan].columns.pc.events) or {}) do
  ...
  for _, g in pairs(groups) do
    ...
    for i = 2, #g do
      ...
      -- setCell would renew the lane its number names without that lane's contents having moved.
      if lost.cell then frame.setCell(lost.cell, 'sampleShadowed', true)
...
do
  ...
  -- The maintained fx-host set for a channel (uuids of on-take .fx notes); computeNoteFxSpans reads it
  -- instead of rescanning columns.
  function index.fxHosts(chan) return fxHosts[chan] end

  -- Resolve a uuid to its live column cell via the seat stamp (byUuid.colEvt), so the fx span cache
  -- reseeks a dirty host without a column walk. see docs/trackerManager.md § Note fx span cache
  ...
  -- computeNoteFxSpans never rescans columns to find hosts.
...
local function freezeRegion(uuid, toGroup)
  ...
  -- through windowForNote, the window set's own builder.
  ...
  if not region then
    ...
    if hostSpec then
      hostRegion = windowForNote(hostSpec, offTakeEnd(hostSpec))
    else
      ...
      if cell and cell.fx then
        ...
        hostRegion = windowForNote(cell, clippedSpanEnd(cell, tm:toLogical(cell.chan, tm:length())))
      end
    end
  end
  ...
  local frozenWindows = perTargetWindows(settled)
  ...
  local function hostDropped(pa)
    for _, cell in ipairs(frame.channels[pa.chan].parked or {}) do
  ...
  if toGroup then thinSeats(frozen.chan, frozenWindows) end
  ...
  return groupMembers(frozen, frozenWindows, promotedUuids)
end
...
local function rebuildExtraColumns(extraColumns, paramAutomation)
  ...
  local grew   = false
  for i = 1, 16 do
    local c    = frame.channels[i].columns
...
local parkedClipEnd = {}   -- uuid -> endppqC (logical); take-scoped, dropped by forgetCaches at the
                           -- take-tier seam, where a take-length change also arrives (mirrors noteFxClipEnd)
...
local function realiseParked(chan, members, takeLenL)
  ...
  for _, m in ipairs(members) do
    ...
    if dirty then
      ...
      local onTake     = nextLaneOnset(frame.channels[chan].columns.notes[m.lane].events, m.ppq)
...
local function renderUnion(field, newParked, toCell)
  for chan = 1, 16 do frame.channels[chan][field] = {} end
  for _, spec in ipairs(newParked) do
    util.add(frame.channels[spec.chan][field], toCell(spec))
  end
end
...
local function rebuildRegionPark(windows, fxParked, prevWindows, noteFxSpans, settleWindows)
  ...
  windows = settleWindows(restoredCells, parkedNotes)
  ...
  do
    ...
    for chan = 1, 16 do
      if dirt.has(chan) then
        for cc, col in pairs(frame.channels[chan].columns.ccs) do
    ...
    renderUnion('parkedCC', newParked, function(spec)
      local ccs = frame.channels[spec.chan].columns.ccs
...
local function rebuildPA()
  ...
  for chan = 1, 16 do
    if dirt.has(chan) then
      local covers = seedCovers(chan)
      for _, cell in ipairs(frame.channels[chan].parkedPA or {}) do
        if covers(cell) then
...
-- How far a note's fx reaches: authored/take ceiling clipped to the strict-next same-lane onset,
-- cached per uuid (noteFxClipEnd), dirt-gated. see docs/trackerManager.md § Note fx span cache
local noteFxClipEnd = {}   -- uuid -> spanEndL (logical); take-scoped, dropped by forgetCaches at the
                         -- take-tier seam, which a take-length change (mm:setLength) reaches too

--shape: computeNoteFxSpans -> { [cell] = spanEndL }; each span starts at its cell's own onset
local function computeNoteFxSpans(extraFxChans, parkedNotes)
  local takeLen = tm:length()
  local noteFxSpans = {}
  -- The stash is the authority on parkedness: the fx-host index lags a park until the tail-walk
  -- commit. see docs/trackerManager.md § Fx window census
  local parked = {}
      ...
      for _, evt in ipairs(col.events) do
        if evt.evType ~= 'pa' then
          if openHosts[1] and evt.ppq > openHosts[1].ppq then
            for _, h in ipairs(openHosts) do noteFxSpans[h] = math.min(noteFxSpans[h], evt.ppq) end
            openHosts = {}
          end
          if evt.fx then
            noteFxSpans[evt] = (evt.endppq == nil or evt.endppq == util.OPEN) and takeLenL or math.min(evt.endppq, takeLenL)
            util.add(openHosts, evt); util.add(hosts, evt)
          end
        end
      end
    end
    for _, h in ipairs(hosts) do noteFxClipEnd[h.uuid] = noteFxSpans[h] end
  end
  ...
  local function perHost(chan, takeLenL, seedList)
    ...
    for uuid in pairs(index.fxHosts(chan)) do
      if not parked[uuid] then
        ...
        local cached = noteFxClipEnd[uuid]
        ...
        local spanEnd = dirty and clippedSpanEnd(cell, takeLenL) or cached
        noteFxClipEnd[uuid] = spanEnd
        noteFxSpans[cell]   = spanEnd
      end
    end
    return true
  end
...
local function rebuildFx(noteExisting, ccExisting, noteFxSpans, windows, fxRegions, notesByHost)
  ...
  local function pbBaseFor(chan, spanSet)
    local base, seen = {}, {}
    for _, cell in ipairs(frame.channels[chan].parkedPb or {}) do
  ...
  local function ccBasesFor(chan, spanSet)
    local bases, seen = {}, {}
    for _, cell in ipairs(frame.channels[chan].parkedCC or {}) do
  ...
  local function nextSameLaneNote(host)
    ...
    local found
    local col = frame.channels[host.chan].columns.notes[host.lane]
    if col then
    ...
    for _, cell in ipairs(frame.channels[host.chan].parked or {}) do
...
local function makeTailRules(ctx)
  ...
  local function settleOnset(e, prev)
    ...
    if e.colEvt and e.colEvt.delay ~= nil then
      ...
      frame.setCell(e.colEvt, 'delayC', util.round(timing.ppqToDelay(shift, res)))
    end
  ...
  local function boundNote(e, laneNext, pitchNext)
    ...
    if e.colEvt then
      ...
      frame.setCell(e.colEvt, 'endppqC', endppqC)
      frame.setCell(e.colEvt, 'endppq', e.endppqL == util.OPEN and util.OPEN or e.endppqL or endppqC)
    end
  end
...
local function rebuildPbs(fxOut, extraColumns)
  ...
  -- channels alone; clean ones reuse their carried pb column. see docs/tuning.md § Absorber reconciliation
  ...
  local function anyDetuneJump(chan)
    ...
    for _, note in ipairs(liveLane1ByChan[chan] or {}) do
      if (note.detune or 0) ~= 0 then return true end
    end
    return false
  end
  ...
  local function deriveChan(chan, pbs, replaceWins, seatSpans)
    ...
    for _, onset in ipairs(onsets) do
      ...
      if ramps then
        ...
        if onset.ppq > 0 then
          ...
          seats[dual] = { cents = cents, ppqL = tm:toLogical(chan, dual), shape = 'linear' }
        end
    ...
    if first and not seats[first.ppq] and not inKeptRange(first.ppq) and inSeatScope(first.ppq) then
      ...
      local pbActive  = next(seats) ~= nil or firstReal ~= nil
                        or (seatSpans ~= nil and anyDetuneJump(chan))
    ...
    local detuneOf = {}
    for _, pb in ipairs(pbs) do detuneOf[pb] = lane1DetuneAt(chan, pb.ppq) end
    perf.start('assign')
...
local function rebuildPipeline(didReload)
  ...
  local currentWindows = assembleWindows(headFxSpans, sources.fxParked or {})
  perf.stop('parkRegions')

  -- Post-settlement span pass: settledFxSpans feeds fx expansion's host set, settledWindows the tail's maps.
  -- see docs/trackerManager.md § Note fx spans and windows
  local settledFxSpans, settledWindows
...
function tm:rebuild(takeChanged)
  ...
  for i = 1, 16 do
    if dirt.wholesale(i) then
      frame.channels[i] = { chan = i, columns = { notes = {}, ccs = {} } }
    elseif dirt.has(i) then
      ...
      frame.channels[i] = { chan = i, columns = { notes = prevCols.notes, ccs = prevCols.ccs,
                                            at = prevCols.at, pc = prevCols.pc },
                      priorPb = prevCols.pb }
    else
      frame.channels[i] = prevChannels[i]
    end
  end
