...
--shape: row = { cluster, label, accent } -- a box row: its chips, the text beside them, its cue
...
function keycaps.new(ctx, dl, theme, wash)
  ...
  -- geometry for drawCluster; a run of word chars shares a cell, a symbol its own.
  ...
  -- A box's geometry over rows already laid out: the width is the wider of the title
  -- and the key column plus the widest label, the height a title row and the rows.
  function caps.box(title, rows)
    ...
    return { title = title, rows = rows, keyW = keyW,
             w = math.max(titleW, keyW + KEY_GAP + labelW) + PAD * 2,
             h = PAD * 2 + lineH * (#rows + 1) + ROW_GAP * #rows }
  end

  -- Draws a box at (x, y) and reports each row's placed cluster, in row order.
  function caps.drawBox(box, x, y)
    ...
    for _, row in ipairs(box.rows) do
      util.add(placed, caps.drawCluster(row.cluster, x + PAD, rowY, row.accent))
