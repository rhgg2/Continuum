...
3. The design doc stays closed — the brief was compiled to be
   ...
   - **Tactical** — the brief needs a decision it didn't settle, but
     ...
     settlement to the user, fold it into the brief as a dated
     amendment, continue. The brief must stay the truth of what is
     being built — a fresh session resuming mid-task reads the brief,
...
4. Done is the brief's own definition plus a green suite → remind to
   commit, once (`/commit` carries the landing bookkeeping). Stopping
   short of done → leave the brief amended to reflect actual state
   before ending.
