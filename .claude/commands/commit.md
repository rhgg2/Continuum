...
2. Comment-hygiene pass on `.lua` files in the diff. Run `tools/comment_hygiene.py` — it flags `--invariant:`/`--contract:`/`--emits:`/`--reaper:` lines >100 chars, `--shape:` lines >400 chars (split prose to `docs/<file>.md` or factor sub-shapes), and contiguous WHY-comment runs >2 lines, only where an added line participates. Fix each violation it names. Longer WHYs move to `docs/<file>.md` with a one-line pointer at the site. Don't audit comments the script didn't flag.
3. `git add -A`.
