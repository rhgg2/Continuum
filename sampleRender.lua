...
--invariant: render + input only — handed sv only, never sm; slot mutations route through sv
--invariant: only frame-local caches + ephemeral drag/preview/rename state live here
...
local function revertPreviewInPlace()
  ...
  sv:syncSlot(pip.track, pip.slot)
...
local function triggerPreviewInPlace(srcPath)
  ...
  if not sv:stageInto(slot, srcPath) then return end
  pip.slot             = slot
  pip.track            = track
...
local function drawStrip(stripW, stripH)
  ...
  if active then
    ...
    sv:setTrim(slot, drag.startF, drag.endF)
...
local function boundTrackName()
  ...
  for _, e in ipairs(sv:listTracks()) do
    if e.track == cur then return e.name end
  end
  return '(no track)'
end
...
function sr:renderBody(_, w, h, dispatch)
  ...
  -- Bottom strip: waveform + trim editor, width gridW (stops at the palette).
  ImGui.SetCursorScreenPos(ctx, ox, oy + topH + GAP)
  if ImGui.BeginChild(ctx, '##sampleStrip', gridW, stripH,
                      ImGui.ChildFlags_None) then
