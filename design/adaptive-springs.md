...
2. The lattice model pinned every interval. A strand's tuning was
...
3. What survives the exchange: the strand and the walk
...
2. Each sonority of the walk takes a **spelling**: coords per member
...
3. Each spelled pair is a **spring**: a quadratic charge on the gap
...
4. The objective is the box summed over the walk, the mistuning summed
...
2. The winner is settled by one joint relaxation over its springs, as
   the lattice settled its winner's offset.
...
2. A ii–V–I of sevenths. The walk returns what an exhaustive search
   over its spelling lists returns — 18.3248, over 13,824 choices —
   within 0.66¢ realised of the lattice's answer.

3. A diminished triad. The lattice places it only 18.8¢ sharp of where
...
4. A dominant seventh and a C minor triad land within 0.6¢ of the
   lattice's placements; the comma pump within 0.7¢ under stiff
   springs, closing a syntonic comma flat.

5. Locality. Clamping one strand 10¢ off its optimum moves later
...
2. The single offset. The lattice stated a passage at one
...
3. The account of a carried strand. A strand present at two onsets is
...
2. Refusal. The lattice refused a passage its moves could not reach;
...
3. Certification at width. The beam is checked against a full
