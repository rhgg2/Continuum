...
- **2026-09-18** — The pass's authority over um's derived records is a set of host uuids per
  ...
  is outside the set survives, and the three readers admit it beside the authored ones.
  `isAuthored` stays for the readers that mean authorship -- the sample stamp, PA column
  containment, the lane-1 nudge gate, and pcSeedSpans' span edges -- and `survivingEvents(ran)` serves
...
- **2026-09-02** — One merge pass over keys and chunks applies a flush's whole drop/put/repack set
  ...
  k splices over an n-key wire cost O(k*n) against buildWire's O(n): deleting an fx note host took
  every tile its chain realised with it, 3585 keys and 212ms of splicing on a take whose whole wire
  re-keys in 5. One pass has no crossover and so no constant to tune, and it dissolves the phase
  ordering the per-key form needed, a batch never packing a dead key because a dropped key is never
  emitted. It costs ~1.1x buildWire when every key moves, which is the price of having no cliff.
  The specs that pinned the three primitives one by one now drive gestures through syncSlots, which
  is the path mm actually takes.
